...
class TestFullShardAccounting(unittest.TestCase):
    def test_collection_skip_is_not_a_case(self) -> None:
        xml = """<testsuite>
          ...
          <testcase classname="tests.desktop" name="">
            <skipped message="collection skipped">missing webview</skipped>
          </testcase>
        </testsuite>"""
    ...
    def test_triage_keeps_timeout_and_async_failure_separate(self) -> None:
        ...
        self.assertEqual(
            classify("Failed: async def functions are not natively supported."), "async_test_configuration"
        )

    def test_changed_parameter_id_is_not_auto_reconciled(self) -> None:
        missing, extra, pairs = reconcile_ids(
            ["tests/a.py::test_zip[clock=1]", "tests/b.py::test_missing"], ["tests/a.py::test_zip[clock=2]"]
        )
    ...
    def test_incomplete_or_not_run_archive_fails_even_when_pytest_exits_zero(self) -> None:
        summary = {"closed": True, "not_run": 0, "results": {"shard-001": {"returncode": 0, "junit_parse_error": None}}}
