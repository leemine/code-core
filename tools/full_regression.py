...
def main() -> int:
    ...
    archive = (
        args.archive or ROOT / "artifacts/test-runs" / f"full-python-{datetime.now(timezone.utc):%Y%m%dT%H%M%SZ}"
    ).resolve()
    ...
    run_command = [
        str(python),
        str(ROOT / "tools/run_full_shards.py"),
        "--repo",
        str(ROOT),
        "--plan",
        str(plan_path),
    ...
    if (archive / "summary.json").exists():
        analyzed = subprocess.run(
            [str(python), str(ROOT / "tools/analyze_full_shards.py"), str(archive)], cwd=ROOT, check=False
        )
    else:
        analyzed = None
