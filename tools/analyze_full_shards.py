...
def classify(message: str) -> str:
    ...
    if (
        ...
        or "import pyarrow" in lower
        or "usable engine" in lower
    ):
...
def inspect(archive: Path) -> dict:
                ...
                records.append(
                    {
                        ...
                        "junit": junit,
                        "owner_suggestion": owner(summary["repo"], case_id),
                    }
                )
            ...
            records.append(
                {
                    ...
                    "junit": junit or "",
                    "owner_suggestion": owner(summary["repo"], nodeid.replace("/", ".")),
                }
            )
            ...
            records.append(
                {
                    "repo": summary["repo"],
                    "shard": shard_id,
                    "case_id": drift["canonical"],
                    "state": "id_drift",
                    "category": "dynamic_parameter_id",
                    "evidence": f"isolated ID: {drift['isolated']}",
                    "junit": junit or "",
                    "owner_suggestion": f"{summary['repo']}/test-infrastructure",
                }
            )
            ...
            records.append(
                {
                    "repo": summary["repo"],
                    "shard": shard_id,
                    "case_id": f"{shard_id}::collection",
                    "state": "collection_error",
                    "category": "collection_or_runner_failure",
    ...
    return {
        ...
        "by_owner_suggestion": dict(by_owner),
        "records": records,
    }
...
def main() -> int:
    ...
    lines.extend(
        [
            "",
            "Owner suggestions are based on test paths, not assigned people.",
