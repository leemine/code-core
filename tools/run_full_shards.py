...
def run_exit_code(summary: dict) -> int:
    return int(
        not summary["closed"]
        or summary["not_run"] != 0
        or any(item["returncode"] != 0 or item["junit_parse_error"] for item in summary["results"].values())
    )
...
def run_shard(
    ...
    with log.open("w", encoding="utf-8") as stream:
        ...
        process = subprocess.Popen(
            command, cwd=root, env=env, stdout=stream, stderr=subprocess.STDOUT, start_new_session=True
        )
        timed_out = False
        try:
    ...
    closed = (
        ...
        and junit.exists()
        and not extra_collection
        and junit_cases == len(actual_collected)
        and junit_cases + len(missing_collection) == len(canonical_cases)
    )
    result = {
        "id": shard_id,
        "planned": len(canonical_cases),
        "files": shard["files"],
        "counts": counts,
        ...
        "log": str(log.relative_to(archive)),
...
def main() -> int:
    ...
    parser.add_argument(
        "--pytest-arg", action="append", default=[], help="explicit pytest option retained when addopts is cleared"
    )
    ...
    summary = (
        ...
        if summary_path.exists()
            ...
            "python": str(python),
            "canonical_collect_log": str(args.collect_log.resolve()),
            "planned_cases": plan["collected_cases"],
    ...
    if args.reconcile_only:
        ...
        for item in summary["results"].values():
            ...
            item["closed"] = (
                item["collect_returncode"] == 0
                and not item["junit_parse_error"]
                and item["junit"] is not None
                and not extra
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        ...
        for future in as_completed(futures):
            ...
            summary["closed"] = len(summary["results"]) == plan["shard_count"] and all(
                item["closed"] for item in summary["results"].values()
            )
            summary["not_run"] = sum(len(item["not_run_cases"]) for item in summary["results"].values())
            summary["id_drift"] = sum(len(item.get("id_drift_cases", [])) for item in summary["results"].values())
            summary["collection_skip_events"] = sum(
                len(item["collection_skips"]) for item in summary["results"].values()
            )
            save_json(summary_path, summary)
            counts = result["counts"]
            print(
                f"{root.name} {result['id']}: {counts} rc={result['returncode']} "
